	...
	<view class="container">
		...
		<view class="" v-if="!searchText.length>0">
			...
			<view v-if="searchList.length>0" class="flexRowStart searchList">
				<view @tap="chooseSearch(item)" class="searchItem color_919397 singleEllipsis" v-for="(item,index) of searchList" :key="index" :class="index==2 || index==5 ||index==8?'':'searchItemChoose'">
					{{$isEmpty(item)?'':item}}
				</view>
			</view>
