	...
	<view class="container">
		<view class="title">
			<view class="kindList" id="list">
				...
				<view class="flexRowStart kindListItem" :class="activeSort?'pickOn':'pickOff'"
				 @tap="chooseSort">
				...
				<view class="flexRowStart kindListItem" :class="activeType?'pickOn':'pickOff'"
				 @tap="chooseType">
					<text>筛选</text>
			...
			<uni-popup ref="sortStatus" type="top">
				...
				<view class="" v-if="activeKind==true">
					<view class="sortAction color_919397 flexRow" :style="{top:topHeight + 'px',maxHeight:maxHeight+'px'}" style="width: 100%; align-items: flex-start;padding: 10rpx 26rpx;">
						<scroll-view class="flexColumnStart" scroll-y="true" style="" :style="{maxHeight:maxHeight+'px'}">
							<view class="flexRowStart" style="flex-wrap: wrap;">
								<view class="singleEllipsis" :class="item.status?'pickChooseOn':'pickChooseOff'" v-for="(item,index) of allTypeList" :key="index" style="width: 204rpx;height:66rpx;margin: 0 14rpx; line-height: 66rpx;text-align: center;margin-bottom: 30rpx;border-radius: 10rpx;font-size: 24rpx;"
								 @tap="firstAll(item,index)">
				...
				<view class="" v-else-if="activeSort==true">
					<view class="flexColumn sortAction" :style="{top:topHeight + 'px'}">
						<view class="sortKind" :class="item.status?'pickOn':'pickSortOff'" v-for="(item,index) of sortList" :key="index"
						 :style="index<2?'border-bottom:0.3rpx solid #F4F4F4':''" @tap="changeSort(item,index)">
				...
				<view class="" v-else-if="activeType==true">
					<view class="sortAction color_303031" :style="{top:topHeight + 'px'}">
						...
						<view class="flexRow chooseBtn">
							<view class="chooseBtnItem" :class="item.status?'pickChooseOn':'pickChooseOff'" v-for="(item,index) of hardLevel"
							 :key="index" @tap="changeHardLevel(item,index)">
								{{$isEmpty(item.label)?'':item.label}}
							</view>
						</view>
						<view class="typeTitle bold">
						...
						<view class="flexRow chooseBtn">
							<view class="chooseBtnItem" :class="item.status?'pickChooseOn':'pickChooseOff'" v-for="(item,index) of examStatus"
							 :key="index" @tap="changeExamStatus(item,index)">
								{{$isEmpty(item.name)?'':item.name}}
							</view>
						</view>
						<view class="typeTitle bold">
						...
						<view class="flexRowStart chooseBtn">
							<view class="chooseBtnItem" :class="item.status?'pickChooseOn':'pickChooseOff'" v-for="(item,index) of examType"
							 :key="index" style="margin-right: 30rpx;" @tap="changeExamType(item,index)">
								{{$isEmpty(item.name)?'':item.name}}
							</view>
						</view>
