	...
	export default {
		...
		methods: {
			...
			confirm() {
				...
				uni.setStorageSync(this.type == 'recruit' ? 'recruitFilter' : 'recruitPartnerFilter', this.data);
				uni.navigateBack();
			}
		},
		onLoad(option) {
			...
			let deFilter = null;
			if (type == 'recruit') {
				deFilter = uni.getStorageSync('recruitFilter')
			} else if (type == 'partner') {
				deFilter = uni.getStorageSync('recruitPartnerFilter')
			}
			if (this.$isEmpty(deFilter)) {
				this.$http.get('dms/recruit_query/condition', ).then(res => {
					let dataList = []
					res.data.data.forEach(item => {
						if (item.type == '0' || item.type == (type == 'recruit' ? '1' : '2')) {
