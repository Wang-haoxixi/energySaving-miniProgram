			...
			<view class="content flexColumn">
				...
				<view class="" style="flex: 1;margin-top: 104rpx">
					...
					<view class="color_919397" style="font-size: 28rpx;line-height: 40rpx;margin-bottom: 48rpx;text-align: center;">
						{{$isEmpty(position)?'暂无职位':position}}
					</view>
				</view>
				<view class="" style="">
	...
	export default {
		...
		onShareAppMessage(res) {
			...
			return {
				...
				path: `/pages/my/information/myCode/myCode?avatar=${this.avatar}&realName=${this.name}&position=${this.position}&id=${this.id}`
			}
		},
		onShareTimeline(res) {
			this.$http.post('admin/mark/save', {
			...
			return {
				title: '我分享了【' + this.realName + '】的二维码，快去看看吧',
		...
		methods: {
			...
			make(id) {
				uQRCode.make({
					...
					text: `http://www.modulestore.cn:63083/?type=canerShare&id=${id}`,
					size: 211,
					margin: 0,
