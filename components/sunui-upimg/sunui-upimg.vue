		...
		<view class="sunui-uploader-files">
			<block v-for="(item, index) in upload_before_list" :key="index">
				<view class="sunui-uploader-file" :class="[item.upload_percent < 100 ? 'sunui-uploader-file-status' : '']" @click="previewImage(index)">
					...
					<image class="sunui-img-removeicon right" @click.stop="removeImage(index)" v-show="upimg_move && oss" :src="oss+'icon_img_canel.png'"></image>
			...
			<view v-show="upload_len < upload_count" hover-class="sunui-uploader-hover" class="sunui-uploader-inputbox" @click="chooseImage"
			 :style="upload_img_wh">
				<image v-if="oss" :src="$ossUrl('publish_add.png')" style="width: 204rpx;height: 204rpx;"></image>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			...
			upload_img_wh: {
				type: String,
				default: 'width:204rpx;height:204rpx;border-radius:16rpx;margin-right: 29rpx;margin-top: 30rpx;'
			},
			// 上传数量
			upload_count: {
				type: [Number, String],
