<template>
	<view class="uni-rate">
		<view :key="index" :style="{ marginLeft: margin + 'rpx' }" @click="_onClick(index)" class="uni-rate__icon" v-for="(star, index) in stars">
			<uni-icons :color="color" :size="size" :type="isFill ? 'star-filled' : 'star'" />
			<!-- #ifdef APP-NVUE -->
			<view :style="{ width: star.activeWitch.replace('%','')*size/100+'rpx'}" class="uni-rate__icon-on">
				<uni-icons style="text-align: left;" :color="activeColor" :size="size" type="star-filled"
				 :background="background" :webkit_background_clip="webkit_background_clip" :webkit_text_fill_color="webkit_text_fill_color"/>
			</view>
			<!-- #endif -->
			<!-- #ifndef APP-NVUE -->
			<view :style="{ width: star.activeWitch,top:-size/2+'rpx' }" class="uni-rate__icon-on">
	...
	export default {
		...
		data() {
			return {
				...
				background:"-webkit-linear-gradient(95deg, #47B48D 0%, #4FC9B3 100%)",
