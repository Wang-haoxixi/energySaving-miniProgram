<template>
	<view v-if="isShow" class="picker">
		...
		<view v-if="type!='time'" class="picker-modal">
			<view class="flexRow" style="padding: 40rpx;">
				<view class="color_303031" style="font-size: 32rpx;line-height: 44rpx;font-weight: 600;">
					截止时间
				</view>
				<view class="color_666769 flexRowStart" style="font-size: 28rpx;line-height: 40rpx;margin-right: -40rpx;">
					<view class="" style="margin-right: 22rpx;">
						重复
					</view>
					<switch color="#F74437" @change.stop="changeStatus" :checked="status" style="transform:scale(0.7)" />
				</view>
			</view>
			...
			<view class="" style="padding: 20rpx 40rpx;" v-show="status">
				<radio-group @change.stop="radioChange" style="position: relative;z-index: 60;">
					<label class="uni-list-cell uni-list-cell-pd flexRow" v-for="(item,index) in items" :key="index" style="background-color: #F4F5F6;padding: 30rpx;border-radius: 30rpx;margin-bottom: 40rpx;">
						<view class="color_666769 bold" style="font-size: 32rpx;line-height: 44rpx;">{{item.value}}</view>
						<view>
							<radio :value="item.value" :checked="item.value==repeatValue" style="transform:scale(0.7)"/>
						</view>
					</label>
				</radio-group>
				<picker-view v-show="visible" indicator-style="height:30px;" :value="scrollValue" @change.stop="bindChange" style="height: 300rpx; width: 100%;">
					<picker-view-column style="height: 300rpx;">
						<view class="item"  style="text-align: center;line-height: 30px;height: 30px;" v-for="(item,index) in repeatDay" :key="index">{{item}}</view>
					</picker-view-column>
				</picker-view>
				<view class="flexRow" style="position: relative;z-index: 20;">
					<view class="button" style="width: 320rpx;margin-right: 30rpx;color: #565B5F;background: #F5f7f8;" @tap.stop="onRepeatCancel">
						取消
					</view>
					<view class="button" style="width: 320rpx;" @tap.stop="chooseRepeat(repeatValue)">
						确定
					</view>
				</view>
			</view>
	...
	export default {
		...
		data() {
			return {
				...
				repeatDayitem:'1天',
				scrollValue:[0],
				repeatDay:['1天','2天','3天','4天','5天','6天','7天','8天','9天','10天',
						...
						'51天','52天','53天','54天','55天','56天','57天','58天','59天','60天',
						'61天','62天','63天','64天','65天','66天','67天','68天','69天','70天',
						'71天','72天','73天','74天','75天','76天','77天','78天','79天','80天',
						'81天','82天','83天','84天','85天','86天','87天','88天','89天','90天',
						'91天','92天','93天','94天','95天','96天','97天','98天','99天',
						],		
			};
		},
		onReady() {
			if(this.chooseDate.includes('重复')){
				// console.log(6)
				this.repeatValue =this.chooseDate
				if(this.chooseDate.includes('天')){
					this.repeatDayitem = parseInt(this.chooseDate)+'天'
					this.visible =true
					this.scrollValue =[parseInt(this.chooseDate)-1]
					this.items[3].value=this.chooseDate
					this.repeatValue =this.chooseDate
				}
			}
		},
		methods: {
			bindChange(e){
				...
				this.repeatDayitem= this.repeatDay[val[0]]
				this.scrollValue = val
				// console.log(this.repeatDayitem)
				this.repeatValue=this.repeatDayitem+'重复'
				this.items[3].value=this.repeatValue
			},
			radioChange(e){
				// console.log(e)
				if(e.detail.value=='自定义重复' || e.detail.value.includes('天')){
					this.visible=true
					this.repeatValue=this.repeatDayitem+'重复'
					this.items[3].value=this.repeatValue
				}else{
					this.items[3].value='自定义重复'
					this.visible=false
					this.repeatDayitem ='1天'
					this.scrollValue = [0]
					this.repeatValue=e.detail.value
				}
			},
			chooseRepeat(value){
				if(!this.repeatValue.includes('天')){
					this.repeatDayitem = '1天'
				}
				this.$emit('getRepeat',this.repeatValue)
			},
			changeStatus(e){
				this.$emit('chooseTime',e)
			},	
			//设置值
			setValue(value) {
				this.date = new Date();
			...
			onRepeatCancel(){
				...
				if(this.chooseDate == ''){
					...
					this.repeatDayitem ='1天'
					// this.items[3].value=this.repeatValue
				}else if(this.chooseDate.includes('天')){
					this.repeatDayitem = parseInt(this.chooseDate)+'天'
					this.repeatValue = this.chooseDate
					this.scrollValue = [parseInt(this.chooseDate)-1]
					this.items[3].value=this.chooseDate
