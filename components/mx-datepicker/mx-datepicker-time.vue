	...
	<view v-if="show" class="picker">
					...
					<view class="picker-modal-footer-info">
						<block>
							<view class="picker-display">
								...
								<view class="picker-display-link" :hover-stay-time="100" hover-class="picker-display-link-active"
								 :style="{color}" @click="onShowTimePicker()">{{timeValueCheck}}</view>
							</view>
						</block>
					</view>
		...
		<view v-if="showTimePicker" class="picker">
			<view class="picker-modal picker-time">
				...
				<picker-view class="picker-modal-time" indicator-class="picker-modal-time-item" :value="timeIndex" @change="onTimeChange">
					<picker-view-column>
						<view v-for="time in timeRange" :key="time.id" :value="time.value">{{time.label}}</view>
					</picker-view-column>
				</picker-view>
				<view class="picker-modal-footer">
					<view class="picker-modal-footer-info">
						<view class="picker-display">
							<text>当前选择</text>
							<text class="picker-display-text">{{timeValue}}</text>
						</view>
					</view>
					<view class="picker-modal-footer-btn">
	...
	export default {
		...
		methods: {
			...
			setValue(value) {
				this.date = new Date();
				if (value) {
					this.checkeds = [DateTools.inverse(value, this.format)]
					this.timeValueCheck = DateTools.format(DateTools.inverse(value, this.format), 'hh:ii')
				} else {
					this.checkeds = [new Date(this.date)]
					this.timeValueCheck = this.timeRange[0].value
				}
			...
			//时间选择变更
			onTimeChange(e) {
				this.timeIndex = [e.detail.value[0]];
			},
			//设置时间选择器的显示状态
			onShowTimePicker() {
				let timeValue = this.timeValueCheck
				for (let i = 0; i < this.timeRange.length; i++) {
					if (this.timeRange[i].value == timeValue) {
						this.timeIndex = [i]
						break;
					}
				}
			...
			//选中日期
			onSelectDate(date) {
				this.checkeds = [new Date(date.dateObj) ]
			...
			onConfirmTime() {
				this.showTimePicker = false;
				this.timeValueCheck = this.timeValue
			},
			//取消
			onCancel() {
			...
			onConfirm() {
				let date = this.checkeds[0]
				let dateStr = DateTools.format(date, this.format)
				let timeDate = new Date(`${DateTools.format(new Date(), 'yyyy-mm-dd')} ${this.timeValueCheck}`)
				date.setHours(timeDate.getHours())
				date.setMinutes(timeDate.getMinutes())
				date.setSeconds(timeDate.getSeconds())
				let result = {
					date,
					value: DateTools.format(date, this.format)
				}
		...
		computed: {
			...
			timeValue () {
				if (this.timeRange && this.timeRange.length > 0) {
					return this.timeRange[this.timeIndex[0]].value
				} else {
					return ''
				}
			}
