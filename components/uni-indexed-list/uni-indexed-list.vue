	...
	<view class="uni-indexed-list" ref="list" id="list">
		...
		<list class="uni-indexed-list__scroll" scrollable="true" show-scrollbar="false">
			<cell v-for="(list, idx) in lists" :key="idx" :ref="'uni-indexed-list-' + idx">
				...
				@refresherrefresh="listPulldown" :refresher-enabled="showRefreshStatus" :refresher-triggered="listStatus">
					<!-- <view :style="{paddingRight: rightWidth + 'px'}"> -->
					<view>
						<view class="" v-if="topList.length>0" style="margin-bottom: 40rpx;">
							<view class="" style="">
								常用组织
							</view>
							<view class="" v-for="(item,index) of topList" :key="index" @tap='goNext(item)'>
								<view class="" style="display: flex;align-items: center;padding: 32rpx 0;">
									...
									<view class="">
										<text class="uni-indexed-list__item-content" style="font-size: 28rpx;">{{item.name}}</text>
									</view>
								</view>
							</view>
						</view>
						<view v-for="(list, idx) in lists" :key="idx" :id="'uni-indexed-list-' + idx">
							<!-- #endif -->
							<uni-indexed-list-item :list="list" :loaded="loaded" :idx="idx" :showSelect="showSelect" :senderId="senderId" :role="role" :transfer="transfer" @itemClick="onClick" @cancel='cancelSo(list)'></uni-indexed-list-item>
							<!-- #ifndef APP-NVUE -->
						</view>
					</view>
		...
		<!-- #endif -->
		<view id="list_index" style="position: absolute;right: 5px;" :style="{ top: winOffsetY - num + 'px', height: winHeight + 'px' }">
...
<script>
	...
	export default {
		...
		methods: {
			listPulldown(){
				// this.listPulldownStatus=true
				this.listStatus = true
				if(!this.$isEmpty(uni.getStorageSync('token'))){
					this.$emit('refresh', this.listStatus)
				}else{
					setTimeout(() => {
						this.listStatus = false
					}, 1000)
